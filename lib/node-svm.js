...
var SVM = function(args){
  ...
  this._node_svm = new addon.NodeSvm();
};
...
SVM.prototype.train = function(problem, callback) {  
  ...
  var error = this._node_svm.setParameters(params);
  ...
  if (typeof callback === 'function' && error){
    ...
    var self = this;
    this._node_svm.train(problem, function(){
      self.labels = self._node_svm.getLabels();
      callback();
    });
  }
};
...
SVM.prototype.predict = function(data) {  
  return this._node_svm.predict(data);
};
SVM.prototype.predictAsync = function(data, callback) {  
  return this._node_svm.predictAsync(data, callback);
};

SVM.prototype.predictProbabilities = function(data) {
  var probs = this._node_svm.predictProbabilities(data);
  var result = {};
...
SVM.prototype.getAccuracy = function(testData, callback) {
  this._node_svm.getAccuracy(testData, callback);
};
...
var readProblem = function(path, callback){
  fs.readFile(path, 'utf8', function (err, data) {
    ...
    lines.forEach(function(line){
      var elts = line.split(" ");
      var node = {};
      node.x = numeric.rep([nb_features],0);
      for (var i = 1; i < elts.length ; i++){
        var index_val = elts[i].split(":");
        var index = parseInt(index_val[0], 10);
        var value = parseFloat(index_val[1]);
