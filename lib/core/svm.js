...
SVM.prototype.evaluate = function (testset) {
    ...
    var mu = this._config.mu,
        sigma = this._config.sigma,
        u = this._config.u;
    var data = _a.map(testset, function (ex) {
        var xNorm = normalizeInput(ex[0], mu, sigma);
        var xRed = numeric.dot(xNorm, u);
        return [xRed, ex[1]];
    });
    ...
    switch (this._config.svmType){
        ...
        case svmTypes.NU_SVC:
        case svmTypes.ONE_CLASS:
            return classification.evaluate(data, this._baseSvm);
        case svmTypes.EPSILON_SVR:
        case svmTypes.NU_SVR:
            return regression.evaluate(data, this._baseSvm);
        default :
