...
SimpleSvm.prototype._loadFromFile = function(path) {
  ...
  // initialize PCA and normailization matrices
  this._mu = args.mu;
...
SimpleSvm.prototype.saveToFile = function(path) {
  this._svm.saveToFile(path+'.tmp');
  var fileContent = fs.readFileSync(path+'.tmp', 'utf8').toString();
  var args = helpers.readLibsvmModelFileContent(fileContent);
  args.normalize = this._normalize;
  args.mu = this._mu;
  args.sigma = this._sigma;
  args.reduce = this._reduce;
  args.retainedVariance = this._retainedVariance;
  args.u = this._u;
...
var loadSvmFromFile = function (path) {
  return new SimpleSvm({file: path});
};
...
exports.NuSVR = NuSVR;
exports.loadSvmFromFile = loadSvmFromFile;